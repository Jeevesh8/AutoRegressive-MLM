...
class BaseFinalLayer(hk.Module):
    ...
    def __call__(self, x):
        raise NotImplementedError("Kindly define __call__(x) function in any class you derive from this one.")

    def __call__(self, x, lengths, labels):
        logits = self.__call__(x)
        
        logits_mask = jnp.where(jnp.arange(labels.shape[1]) < lengths, 1., 0.)
        logits = jax.vmap(jnp.multiply, (None,2), 2)(logits_mask, logits)
        labels = hk.one_hot(original_batch, self.hidden_size)
